...
export function PublicSiteClient({ slug }: { slug: string }) {
  ...
  if (status === "loading") return <main className="loading"><div className="spinner" aria-label="正在加载" /></main>;
  if (status === "locked") {
    return (
      <main className="unlock-page">
        <form className="panel unlock-panel" onSubmit={unlock}>
          ...
          <h1>这个导航需要密码</h1>
          <p className="muted">请输入创建者提供的访问密码。</p>
          <div className="field"><label htmlFor="unlock-password">访问密码</label><input id="unlock-password" type="password" autoFocus value={password} onChange={(event) => setPassword(event.target.value)} /></div>
          {error && <p className="field-error">{error}</p>}
          <button className="button" disabled={unlocking || !password}>{unlocking ? "正在验证…" : "进入导航"}</button>
        </form>
      </main>
    );
  }
  if (status === "missing" || !site) return <main className="error-state"><h1>导航站不可用</h1><p className="muted">链接可能已失效、站点已删除或被停用。</p></main>;
