...
import {
  ArrowDown, ArrowUp, Copy, Download, Eye, FileUp, KeyRound, Link2, Pencil,
  Plus, RotateCw, Save, Settings2, Trash2,
} from "lucide-react";
...
export function ManageSiteClient({ slug }: { slug: string }) {
    ...
    async function connect() {
      ...
      try {
        if (key) {
          ...
          setCsrf(sessionStorage.getItem(`teamnav_csrf_${slug}`) ?? "");
        }
        await loadSite();
        setStatus("ready");
  ...
  async function moveCategory(index: number, offset: number) {
    ...
    try { await write(`/api/v1/manage/sites/${slug}/categories/reorder`, "PUT", next.map((item, order) => ({ id: item.id, sort_order: order }))); await loadSite(); }
  ...
  async function moveLink(category: Category, index: number, offset: number) {
    const next = [...category.links]; const target = index + offset;
    if (target < 0 || target >= next.length) return;
    [next[index], next[target]] = [next[target], next[index]];
    try { await write(`/api/v1/manage/sites/${slug}/links/reorder`, "PUT", next.map((item, order) => ({ id: item.id, sort_order: order }))); await loadSite(); }
      ...
      <div className="manage-layout">
        <section className={`manage-editor ${mobileView === "preview" ? "mobile-hidden" : ""}`}>
            ...
            <div className="editor-content">
              ...
              <div className="manage-categories">
                {site.categories.map((category, index) => (
                  <article className="manage-category" key={category.id}>
                    <div className="manage-category-head">
                      <input className="category-icon-input" value={category.icon} onChange={(event) => patchCategory(category.id, { icon: event.target.value })} />
                      <input className="category-name-input" value={category.name} onChange={(event) => patchCategory(category.id, { name: event.target.value })} />
                      <button className="icon-button" title="上移" onClick={() => moveCategory(index, -1)} disabled={index === 0}><ArrowUp size={15} /></button>
                      <button className="icon-button" title="下移" onClick={() => moveCategory(index, 1)} disabled={index === site.categories.length - 1}><ArrowDown size={15} /></button>
                      <button className="icon-button" title="保存分类" onClick={() => updateCategory(category)}><Save size={15} /></button>
                      <button className="icon-button danger-icon" title="删除分类" onClick={() => removeCategory(category)}><Trash2 size={15} /></button>
                    </div>
                    <div className="manage-links">
                      {category.links.map((link, linkIndex) => (
                        <div className="manage-link" key={link.id}><span className="mini-icon">{link.icon}</span><div><strong>{link.name}</strong><small>{link.url}</small></div><button className="icon-button" title="上移" onClick={() => moveLink(category, linkIndex, -1)} disabled={linkIndex === 0}><ArrowUp size={14} /></button><button className="icon-button" title="下移" onClick={() => moveLink(category, linkIndex, 1)} disabled={linkIndex === category.links.length - 1}><ArrowDown size={14} /></button><button className="icon-button" title="编辑" onClick={() => setEditingLink(link)}><Pencil size={14} /></button><button className="icon-button danger-icon" title="删除" onClick={() => removeLink(link)}><Trash2 size={14} /></button></div>
                      ))}
                      {category.links.length === 0 && <p className="empty-inline">这个分类还没有链接</p>}
                    </div>
                  </article>
                ))}
              </div>
          ...
          <details className="editor-section" open>
            ...
            <form className="editor-content form-grid" onSubmit={addLink}>
              ...
              <div className="field"><label>名称</label><input required value={linkDraft.name} onChange={(event) => setLinkDraft({ ...linkDraft, name: event.target.value })} /></div>
              ...
              <div className="field span-2"><label>URL</label><input required type="url" placeholder="https://" value={linkDraft.url} onChange={(event) => setLinkDraft({ ...linkDraft, url: event.target.value })} /></div>
              ...
              <div className="field"><label>标签（逗号分隔）</label><input value={linkDraft.tags} onChange={(event) => setLinkDraft({ ...linkDraft, tags: event.target.value })} /></div>
          ...
          <details className="editor-section">
            ...
            <div className="editor-content form-stack">
              <div className="field"><label>批量添加，每行“名称 | URL | 描述”</label><textarea value={batch} onChange={(event) => setBatch(event.target.value)} placeholder="GitHub | https://github.com | 代码托管" /></div>
          ...
          <details className="editor-section danger-zone">
            ...
            <div className="editor-content form-stack"><button className="button secondary" onClick={rotateKey}><RotateCw size={16} /> 轮换私密编辑链接</button><button className="button danger" onClick={deleteSite}><Trash2 size={16} /> 删除这个站点</button></div>
          </details>
        </section>
